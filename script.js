...
window.onload = function(){
    console.log("Welcome to Learn CAD with Er. Arun Banjare - LMS & Portal Active");
};
...
// 17. Multi-Level Folders & Normal PDF Viewer (Google Drive / Direct Preview)
                ...
                Object.keys(subjectsMap[subj]).sort().forEach(unit => {
                    ...
                    subjectsMap[subj][unit].forEach(m => {
                        ...
                        if (m.type === 'video') {
                            ...
                            listHtml += `<div style="padding:8px; border:1px solid #e2e8f0; border-radius:6px; display:flex; justify-content:space-between; align-items:center; background:#fff;">
                                <div><b>📄 ${m.title}</b></div>
                                <div><button onclick="openNormalPdfViewer('${m.link}', '${m.title}')" class="btn" style="padding:5px 12px; font-size:0.8rem; background:#17a2b8;">View PDF</button>${editBtnHtml}${deleteBtnHtml}</div>
                            </div>`;
                        }
                    });
                    listHtml += `</div></div>`;
...
function openNormalPdfViewer(pdfUrl, title) {
    ...
    document.getElementById('modal-body').innerHTML = `
        <div style="width: 100%; height: 500px; background: #fff; border-radius: 8px; overflow: hidden;">
